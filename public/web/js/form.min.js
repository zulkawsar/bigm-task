...
$(function() {
        ...
        $("#contactForm input,#contactForm textarea").jqBootstrapValidation({
            ...
            submitSuccess: function(a, t) {
                t.preventDefault();
                var e = $("input#emailsubscribe").val(),
                    o = e;
                	o.indexOf(" ") >= 0 && (o = e.split(" ").slice(0, -1).join(" ")), $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    cache: !1,
                    success: function() {
                        $("#msg").html("<div class='alert alert-success'>"), $("#msg > .alert-success").html("<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;").append("</button>"), $("#msg > .alert-success").append("<strong>Your email has been sent. </strong>"), $("#success > .alert-success").append("</div>"), $("#subscribeForm").trigger("reset")
                    },
                    error: function() {
                        $("#msg").html("<div class='alert alert-danger'>"), $("#msg > .alert-danger").html("<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;").append("</button>"), $("#msg > .alert-danger").append("<strong>Sorry Please type valid email or try again later!"), $("#msg > .alert-danger").append("</div>"), $("#subscribeForm").trigger("reset")
                    }
                })
            },
            filter: function() {
